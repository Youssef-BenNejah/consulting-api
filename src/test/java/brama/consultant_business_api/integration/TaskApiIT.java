...
class TaskApiIT extends BaseIntegrationTest {
    ...
    void taskCrudFlow() {
        ...
        ResponseEntity<String> created = post("/api/v1/tasks", taskRequest);
        assertThat(created.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        String taskId = idFrom(created);

        ResponseEntity<String> list = get("/api/v1/tasks?projectId=" + projectId);
        assertThat(list.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(meta(list).path("total").asInt()).isGreaterThanOrEqualTo(1);

        ResponseEntity<String> deleted = delete("/api/v1/tasks/" + taskId);
